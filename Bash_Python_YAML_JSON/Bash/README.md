...
| ------------- |---------|-------------------------------------------------------------------------------------------------------------------|
| `c`  | a+b     | присвоили переменной (с) текст "a+b", так как это строка по умолчанию                                             |
| `d`  | 1+2     | аналогично присвоили переменной (d) текст "1+2", так получили значение переменных (a и b) за счет использования $ |
| `e`  | 3       | за счет использования скобок получили значение арифметической операции сложения значений переменных               |
...
### Ваш скрипт:
```bash
maxsizefile=10000 #добавил ограничение на размер log-файла
while ((1==1)) #добавил закрывающую скобку
do
    curl https://localhost:4757
if (($? != 0))
    then
	if [ $(wc -bytes < curl.log) -gt $maxsizefile ] # при переполнении файла производим его перезапись
            then
                echo "Update file" > curl.log
        fi 
	date >> curl.log
	else # добавил завершение скрипта при доступности сервиса
        exit
fi
done
``` 
...
do 
    ...
    do 
        ...
        if (($?==0)) 
        then 
            echo $ip "OK" >> hosts.log 
        else 
            echo $ip "Error" >> hosts.log 
        fi 
    done
done
